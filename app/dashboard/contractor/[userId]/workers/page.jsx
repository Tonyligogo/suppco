...
const Workers = () => {
  ...
  const { data: branches } = useCompanySites();
  ...
  if (isPending) {
    return (
      <div className="pt-6">
        <Header
          title="All Workers"
          description="Manage your workers across all sites."
        />
        <TableSkeleton />
      </div>
    );
  ...
  return (
    <div className="space-y-6 pt-4 relative">
      ...
      <DataTable
        ...
        searchPlaceholder="Search workers by name, email, role, or site..."
        onRowAction={handleRowAction}
        rowActions={[
          { action: "assignBranch", label: "Assign site", icon: SquarePen },
      ...
      <DynamicDialog
        ...
        title={`Assign site — ${selectedEmployee?.user_detail ?? ""}`}
        description="Select a site to assign this worker to. This will remove any existing site assignment."
        fields={assignFields}
        onSubmit={handleAssignSubmit}
        submitText={isAssigning ? "Assigning..." : "Assign site"}
      />
      <ConfirmDialog
        open={unassignDialogOpen}
        onOpenChange={setUnassignDialogOpen}
        title={`Unassign ${employeeToUnassign?.name ?? "this worker"}?`}
        description={
          <>
            This will remove{" "}
            ...
            from their current site. You can reassign them at any time.
          </>
        }
